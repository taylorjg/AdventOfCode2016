class BotGraph(botMap: Map[Int, Bot], outputMap: Map[Int, (Bot, Boolean)]) {
  ...
  def connectBots(fromBotNumber: Int, toBotNumber: Int, isLow: Boolean): BotGraph = {
    ...
    val toBot2 = if (fromBot.complete) {
      val value = if (isLow) fromBot.low.get else fromBot.high.get
      toBot.addValue(value)
    } else toBot
    new BotGraph(botMap.updated(fromBotNumber, fromBot).updated(toBotNumber, toBot2), outputMap)
  }

  def addOutput(botNumber: Int, outputNumber: Int, isLow: Boolean): BotGraph =
    botMap get botNumber match {
      case Some(bot) => new BotGraph(botMap, outputMap + (outputNumber -> (bot, isLow)))
      case None =>
        val bot = new Bot(botNumber, Seq())
        new BotGraph(botMap + (botNumber -> bot), outputMap + (outputNumber -> (bot, isLow)))
    }
